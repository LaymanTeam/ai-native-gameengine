...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    // Prefer a spec generated by the Studio front door (stored under forge:<slug>);
    // otherwise derive one from the library card's palette.
    let spec = specFromPalette(game.title, game.palette);
    try {
      const stored = localStorage.getItem(`forge:${id}`);
      if (stored) spec = JSON.parse(stored) as typeof spec;
    } catch { /* ignore */ }
